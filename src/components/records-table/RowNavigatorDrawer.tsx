...
import { ChevronLeft, ChevronRight, Eye, Pencil, XCircle } from "lucide-react";
...
export function RowNavigatorDrawer({
  ...
  useEffect(() => {
    if (currentRecord && isOpen) {
      ...
      columns.forEach((col) => {
        values[col.key] = currentRecord.data[col.key];
      });
      setEditValues(values);
      setJsonErrors({});
    }
  }, [currentIndex, currentRecord, columns, isOpen]);
  ...
  const handleKeyDown = useCallback(
    ...
    [isOpen, editMode, currentIndex, records.length, onIndexChange]
  );
  ...
  const handleSaveField = (col: Column) => {
    ...
    if (col.type === "date" || col.type === "datetime") {
      ...
      if (parsed !== null) {
        onUpdateCell(currentRecord.id, col.key, parsed);
      }
    } else if (col.type === "json") {
      const formatted = normalizeAndFormatJSON(value);
      onUpdateCell(currentRecord.id, col.key, formatted);
    } else if (col.type === "number") {
      const num = value === "" ? null : Number(value);
      onUpdateCell(currentRecord.id, col.key, num);
    } else {
      onUpdateCell(currentRecord.id, col.key, value);
    }
  };

  const getPresentableValue = (
  ...
  const formatViewValue = (col: Column, value: unknown): React.ReactNode => {
    ...
    switch (col.type) {
      ...
      default:
        return <span className="text-sm">{typeof value === "object" ? JSON.stringify(value, null, 2) : String(value)}</span>;
    }
  };
  ...
  const stateIndicator = getRowStateIndicator(currentRecord.state);
        ...
        <div className="flex items-center justify-between bg-slate-50 rounded-lg p-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-slate-700">
              Row {currentIndex + 1} of {records.length}
            </span>
            {stateIndicator && (
              <span
          ...
          {columns.map((col) => (
            ...
            >
              ...
              <div className="px-3 py-2.5">
                {editMode ? renderEditField(col) : formatViewValue(col, currentRecord.data[col.key])}
              </div>
            </div>
          ))}
        </div>
